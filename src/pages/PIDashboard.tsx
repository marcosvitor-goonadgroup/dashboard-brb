...
const PIHeader = ({ campaignName, piNumber }: { campaignName: string; piNumber: string }) => (
  <header className="w-full bg-[#153ece] rounded-[34px] px-8 py-6 mb-6">
    <div className="flex items-center gap-6 min-w-0">
      <div className="flex items-center gap-3 shrink-0">
        <img src={logobrb} alt="BRB" className="h-10 w-auto brightness-0 invert" />
        <img src={adDeskWhite} alt="AD Desk" className="h-8 w-auto" />
      </div>
      <div className="border-l border-white/30 pl-6 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <a
            href={`/${toSlug(campaignName)}`}
            className="text-white/70 text-xs font-medium truncate max-w-[260px] hover:text-white transition-colors"
          >
            {campaignName}
          </a>
          <span className="text-white/40 text-xs">/</span>
        ...
        <p className="text-white/60 text-xs mt-0.5">Painel de PI</p>
      </div>
    </div>
  </header>
);

const PIDashboardContent = ({ campaignSlug, piSlug }: PIDashboardProps) => {
  const { loading, error, filteredData, data } = useCampaign();
  ...
  const campaignName = useMemo(() => {
    const found = data.find(d => toSlug(d.campanha || '') === campaignSlug);
  ...
  const piData = useMemo(
    () =>
      filteredData.filter(
        d => toSlug(d.campanha || '') === campaignSlug && d.numeroPi === piSlug
      ),
    [filteredData, campaignSlug, piSlug]
  );
  ...
  return (
    <div className="min-h-screen bg-[#f1f1f1] relative">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-3 sm:px-6 pt-3 sm:pt-6 pb-3 sm:pb-6">
        <PIHeader campaignName={campaignName} piNumber={piSlug} />
...
const PIDashboard = ({ campaignSlug, piSlug }: PIDashboardProps) => (
  <CampaignProvider>
    <PIDashboardContent campaignSlug={campaignSlug} piSlug={piSlug} />
  </CampaignProvider>
);
