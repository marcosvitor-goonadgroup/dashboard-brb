...
const CampaignHeader = ({ campaignName }: { campaignName: string }) => (
    ...
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-6 min-w-0">
        ...
        <div className="border-l border-white/30 pl-6 min-w-0">
          ...
          <p className="text-white/60 text-xs mt-0.5">Painel de Campanha</p>
        </div>
      </div>
    </div>
  </header>
...
const CampaignDashboardContent = ({ campaignSlug }: CampaignDashboardProps) => {
  ...
  const campaignData = useMemo(
    () => filteredData.filter(d => toSlug(d.campanha || '') === campaignSlug),
    [filteredData, campaignSlug]
  );
  ...
  const previousPeriodMetrics = useMemo(() => {
    ...
    const prev = campaignData.filter(i => i.date >= fourteenDaysAgo && i.date < sevenDaysAgoFromMaxDate);
  ...
  useEffect(() => { setSelectedPI(null); }, [periodFilter]);
  ...
  return (
    <div className="min-h-screen bg-[#f1f1f1] relative">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-3 sm:px-6 pt-3 sm:pt-6 pb-3 sm:pb-6">
        <CampaignHeader campaignName={campaignName} />
...
const CampaignDashboard = ({ campaignSlug }: CampaignDashboardProps) => (
  <CampaignProvider>
    <CampaignDashboardContent campaignSlug={campaignSlug} />
  </CampaignProvider>
);
